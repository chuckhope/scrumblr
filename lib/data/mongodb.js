var Db = require('mongodb').Db;
BSON = require('mongodb').BSONNative,
conf = require('../../config.js').database;
const MongoClient = require('mongodb').MongoClient;

var db = function(callback)
{
	this.rooms = false;
	this.users = false;
	var t = this;

	const client = new MongoClient(conf.connUrl, { useNewUrlParser: true });
	client.connect(err => {
		const collection = client.db("scrumblr").collection("rooms");
		const users = client.db("scrumblr").collection("users");
		collection.createIndex([['name',1]],false,function() {});
		t.rooms = collection;
		t.users = users;
		callback();
	});
}

db.prototype = {
	// Users commands
	createUser(user, callback) {
		this.users.updateOne(
			{username:user.username},
			{$set:{password:user.password,displayName:user.displayName,organization:'unknown',department:'unknown',team:'unknown',role:'unknown',email:'unknown'}},
			{upsert:true},
			callback
		);
	},
	getUser: function(username, callback)
	{
		this.users.findOne(
			{username:username},
			function(err, user) {
				if(user) {
					callback(user);
				} else {
					callback();
				}
			}
		);
	},
	getUserList: function(callback)
    {
        this.users.find({}, { projection: { password:0 } }).toArray(
            function(err, userList) {
                callback(userList);
            }
        );
    },
	checkIfUserExists: function(user, callback)
	{
		let userInput = user;
		console.log(user);
		this.users.findOne(
			{username:userInput.username},
			function(err, user) {
				console.log(user);
				if(user) {
					console.log("user found");
					callback(true);
				} else {
					console.log("user not found");
					callback(false);
				}
			}
		);
	},
	getBoardsByUsername: function(username, callback)
	{
		this.rooms.find({},{}).toArray(
            function(err, list) {
				let returnList = [];
				for(let item of list) {
					let toAdd = false;
					if(item.cards) {
						Object.entries(item.cards).forEach(([key, value]) => {
							if(key.includes("card") && value.assignee == username) {
								toAdd = true;
								return false;
							}
							
						});
						if(toAdd) {
							returnList.push(item);
							toAdd = false;
						}
					}
				}
                callback(returnList);
            }
        );
	},
	
	//Update user profile
	updateUser: function(username, user)
	{
		this.users.updateOne(
			{username:username},
			{$set:{displayName:user.displayName,password:user.password,organization:user.organization,department:user.department,team:user.team,role:user.role,email:user.email}},
			{upsert:true}
		);
	},

	clearRoom: function(room, callback)
	{
		this.rooms.deleteOne({name:room},callback);
	},

	// theme commands
	setTheme: function(room, theme)
	{
		this.rooms.updateOne(
			{name:room},
			{$set:{theme:theme}}
		);
	},

	getTheme: function(room, callback)
	{
		this.rooms.findOne(
			{name:room},
			{theme:true},
			function(err, room) {
				if(room) {
					callback(room.theme);
				} else {
					callback();
				}
			}
		);
	},

	// Column commands
	createColumn: function(room, name, callback)
	{
		this.rooms.updateOne(
			{name:room},
			{$push:{columns:name}},
			{upsert:true}
			,callback
		);
	},

	getAllColumns: function(room, callback)
	{
		this.rooms.findOne({name:room},{columns:true},function(err, room) {
			if(room) {
				callback(room.columns);
			} else {
				callback();
			}
		});
	},

	deleteColumn: function(room)
	{
		this.rooms.updateOne(
			{name:room},
			{$pop:{columns:1}}
		);
	},

	setColumns: function(room, columns)
	{
		this.rooms.updateOne(
			{name:room},
			{$set:{columns:columns}},
			{upsert:true}
		);
	},

	// Card commands
	createCard: function(room, id, card, callback)
	{
		var doc = {};
		doc['cards.'+id] = card;
		this.rooms.updateOne(
			{name:room},
			{$set:doc},
			{upsert:true}
			,callback
		);
	},

	cardUpdate: function(room, id, card)
	{
		var doc = {};
		doc['cards.'+id+'.colour'] = card.colour;
		doc['cards.'+id+'.rot'] = card.rot;
		doc['cards.'+id+'.x'] = card.x;
		doc['cards.'+id+'.y'] = card.y;
		doc['cards.'+id+'.text'] = card.text;
		doc['cards.'+id+'.assignee'] = card.assignee;
		doc['cards.'+id+'.sticker'] = card.sticker;
		doc['cards.'+id+'.rhrs'] = card.rhrs;
		this.rooms.updateOne(
			{name:room},
			{$set:doc},
			//{upsert:true}
		);
	},
	
	createRemainhrs: function(room, id, remainhrs, callback)
	{
		var doc = {};
		uniqueID = remainhrs.id;
		rid = 'remainhrs'+ uniqueID;
		doc['cards.'+id+'.remainhrs.'+rid] = remainhrs;
		this.rooms.updateOne(
			{name:room},
			{$set:doc},
			{upsert:true}
			,callback
		)
	},

	updateRemainhrs: function(room, id, remainhrs, callback)
	{
		var doc = {};
		uniqueID = remainhrs.id;
		rid = 'remainhrs'+ uniqueID;
		doc['cards.'+id+'.remainhrs.'+rid] = remainhrs;
		this.rooms.updateOne(
			{name:room},
			{$set:doc},
			{upsert:true}
			,callback
		);
	},

	getAllCards: function(room, callback)
	{
		this.rooms.findOne({name:room},{cards:true},function(err, room) {
			if(room) {
				callback(room.cards);
			} else {
				callback();
			}
		});
	},

	cardEdit: function(room, id, text)
	{
		var doc = {};
		doc['cards.'+id+'.text'] = text;
		this.rooms.updateOne(
			{name:room},
			{$set:doc}
		);
	},

	cardSetXY: function(room, id, x, y, callback)
	{
		var doc = {};
		doc['cards.'+id+'.x'] = x;
		doc['cards.'+id+'.y'] = y;
		this.rooms.updateOne(
			{name:room},
			{$set:doc}
			,callback
		);
	},

	deleteCard: function(room, id, callback)
	{
		var doc = {};
		doc['cards.'+id] = true;
		this.rooms.updateOne(
			{name:room},
			{$unset:doc}
			,callback
		);
	},

	addSticker: function(room, cardId, stickerId)
	{
		var doc = {};
		doc['cards.'+cardId+'.sticker'] = stickerId;
		this.rooms.updateOne(
			{name:room},
			{$set:doc}
		);
	},
	getBoardSize: function(room, callback) {
		this.rooms.findOne(
			{name:room},
			function(err, room) {
				if(room) {
					callback(room.size);
				} else {
					callback();
				}
			}
		);		
	},
	setBoardSize: function(room, size) {
		this.rooms.updateOne(
			{name:room},
			{$set:{'size':size}}
		);
	}
};
exports.db = db;
